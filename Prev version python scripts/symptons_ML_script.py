...
def apply_model_with_ROC( X, Y, model2, file, if_rand_forest):
    ...
    #apply_cross_validation_roc(X,Y, model2)
...
def apply_cross_validation_roc(X,y, classifier):
    symptom_result_classes = []
    for n in range( len(Patient.sql_symptoms) ):
        symptom_result_classes.append(n)
    ...
    for train, test in cv.split(X, y):
        probas_ = classifier.fit(ml4hX_multiclass[train], yRoc[train]).predict_proba(ml4hX_multiclass[test])
        # Compute ROC curve and area the curve
        fpr, tpr, thresholds = roc_curve(yRoc[test], probas_[:, 1])
